...
class CardDefault extends ComponentStyled {
  __html__ = `
    <div class="d-flex flex-column justify-content-around h-100 w-100 shadow rounded p-2">
      ...
      <!-- ✅ APKDOWNLOAD ELIMINADO: ahora se maneja con el botón superior (.export-config) y modal inline -->
    </div>`;
  ...
  setOnBtnCreateClick(callback) {
    const el = this.element.querySelector(".__create");
    if (!el) return;
    el.addEventListener("click", callback);
  }

  setOnBtnImportClick(callback) {
    const el = this.element.querySelector(".__import");
    if (!el) return;
    el.addEventListener("click", callback);
  }

  setOnBtnSyncClick(callback) {
    const el = this.element.querySelector(".__sync");
    if (!el) return;
    el.addEventListener("click", callback);
  }
}
