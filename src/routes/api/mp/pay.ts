...
export default {
  ...
  handler: async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (!MP_ACCESS_TOKEN || MP_ACCESS_TOKEN.includes('<<APP_USR-292459445257292-010909-ad9da859bf8eb657422b278edbbef85f-517943228>>')) {
    ...
    const payment = await prisma.payment.create({
      data: {
        ...
        currency: 'ARS',
        external_ref: null,
        metadata: JSON.stringify({ plan_code: plan.code, months: plan.months, price_ars: plan.price_ars }),
      },
    });
      ...
      items: [
        {
          title: plan.name,
          quantity: 1,
          currency_id: 'ARS', // 🔁 CAMBIAR_A_ORIGINAL si no es ARS
